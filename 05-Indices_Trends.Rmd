...
This saved workspace includes two objects: 1. jags_data - the data object created with the `prepare_jags_data()` function.

2.  jags_mod - the model output created with the `run_model()` function.

```{r IT1}
load("data/jags_mod_full_Scarlet_Tanager.RData")
...
Setting `slope = TRUE` adds the slope-trend calculation to the output dataframe. 
